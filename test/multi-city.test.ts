...
import {
  AirportCodeSchema, DateStringSchema, FlightLegSchema,
  type AirportCode, type DateString,
} from "../src/domain"
import {
  chooseMultiCityOption, fetchMultiCityItinerary, startMultiCityPicker,
  type MultiCityParams,
} from "../src/services/multi-city"
...
const providerEntry = (options: {
  from: string; to: string; name: string; price: number; carrier: string; number: string; token: string
}): unknown => {
  const flight = new Array(23).fill(null)
...
const SEARCH_PAGE_HTML = [
  `{"cfb2h":"boq_test_bl","FdrFJe":"-42"}`,
  `"H${"a".repeat(40)}AAAAAG${"b".repeat(10)}"`,
].join("\n")
...
const shoppingResponse = (): string => {
  ...
  const leg2 = [
    providerEntry({ from: "LHR", to: "CDG", name: "Air France", price: 95, carrier: "AF", number: "1681", token: "TOK-AF" }),
    null,
  ]
  ...
  HttpClient.make((request) =>
    Effect.succeed(
      HttpClientResponse.fromWeb(
        request,
        new Response(request.method === "GET" ? SEARCH_PAGE_HTML : shoppingResponse())
      )
    )
  )
)
...
const run = <A, E>(effect: Effect.Effect<A, E, HttpClient.HttpClient>) =>
  Effect.runPromise(effect.pipe(Effect.provide(FakeHttpClient)) as Effect.Effect<A, E>)
...
describe("multi-city picker", () => {
  ...
  test("choosing walks legs and completes with flights paired to their legs", async () => {
    const itinerary = await run(Effect.gen(function* () {
