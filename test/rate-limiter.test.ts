...
const runWithLimiter = <A>(config: Parameters<typeof RateLimiterLive>[0], body: (acquire: () => Effect.Effect<void, unknown>) => Effect.Effect<A>) =>
  Effect.runPromise(
    Effect.gen(function* () {
...
describe("RateLimiterLive", () => {
  test("rejects the request that exceeds the window", async () => {
    const exits = await runWithLimiter({ maxRequests: 2, windowMs: 60_000, minDelay: 0 }, (acquire) =>
      Effect.gen(function* () {
  ...
  test("concurrent acquires cannot both take the last slot", async () => {
    const exits = await runWithLimiter({ maxRequests: 1, windowMs: 60_000, minDelay: 0 }, (acquire) =>
      Effect.all([acquire().pipe(Effect.exit), acquire().pipe(Effect.exit), acquire().pipe(Effect.exit)], { concurrency: "unbounded" }),
    )
  ...
  test("failure carries the typed RateLimitExceeded reason", async () => {
    const exit = await runWithLimiter({ maxRequests: 0, windowMs: 60_000, minDelay: 0 }, (acquire) => acquire().pipe(Effect.exit))
