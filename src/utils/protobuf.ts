...
// ---------------------------------------------------------------------------
// Module-level schema & lookups (created once, reused on every encode call)
// ---------------------------------------------------------------------------
...
const PROTO_ROOT = protobuf.Root.fromJSON({
  nested: {
    ...
    Seat: {
      values: { UNKNOWN_SEAT: 0, ECONOMY: 1, PREMIUM_ECONOMY: 2, BUSINESS: 3, FIRST: 4 }
    },
    Trip: {
      values: { UNKNOWN_TRIP: 0, ROUND_TRIP: 1, ONE_WAY: 2, MULTI_CITY: 3 }
    },
    Passenger: {
      values: { UNKNOWN_PASSENGER: 0, ADULT: 1, CHILD: 2, INFANT_IN_SEAT: 3, INFANT_ON_LAP: 4 }
    }
  }
})

const INFO_TYPE = PROTO_ROOT.lookupType("Info")
 ...
 * The tfs parameter is a URL-safe Base64-encoded Protocol Buffer.
 */
export const encodeFlightSearch = (
  ...
  Effect.try({
    try: () => {
      ...
      // Build flight data (date stays YYYY-MM-DD as Google expects)
      ...
      const message = INFO_TYPE.create({
        data,
        seat: SEAT_MAP[seat],
        passengers: passengerArray,
        trip: TRIP_MAP[tripType]
      })

      const buffer = INFO_TYPE.encode(message).finish()
